...
// Initialize the game
let guessedWord = Array(secretWord.length).fill(" ");
...
// if "Enter" key pressed it will submit
document.addEventListener("keydown", function(event) {
  if (event.key === "Enter") {
    submitGuess();
  }
});
// function to handle the players guess
...
// make sure if player guess is not numeric
function isValidGuess(guess) {
return /^[a-zA-Z]{5}$/.test(guess);
}
// check if players guess includes the letter     
function checkGuess(guess) {
  ...
  for (let i = 0; i < secretWord.length; i++) {
    if (guess[i] === secretWord[i]) {
      ...
      alert('the word includes ' +  '"' + guess[i] + '" ');
      feedback.misplaced++;
      }
    }
    
    if (feedback.correct != 0 && feedback.misplaced != 0) {
      feedback.incorrectGuesses++;
    }
    
  displayWord(guessedWord);
  updateFeedback(feedback);
          
  return guessedWord.join("") === secretWord;
}
// updating the players guess
function updateFeedback(feedback) {
  const feedbackElement = document.getElementById("feedback");
  feedbackElement.textContent = `Misplaced: ${feedback.misplaced}, Incorrect Guesses: ${incorrectGuesses++}/6`;
}
// display the letters in a container
function displayWord(word) {
  const wordContainer = document.getElementById("word-container");
...
function resetGame() {
  ...
  displayWord(guessedWord);
  incorrectGuesses = 0;
  updateFeedback({ correct: 0, misplaced: 0 });
